...
zokou({
  ...
  if (arg[0]) {
    ...
    if (action === "on") {
      ...
      await zk.sendMessage(origineMessage, "The command anti-delete is for active.");
      return;
    } else if (action === "off") {
      antiDeleteActive = false;
      await zk.sendMessage(origineMessage, "The command anti-delete is for deactive.");
      return;
    }
  }
  ...
  if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0 && (conf.ADM).toLowerCase() === 'yes') {
    ...
    try {
      ...
      const caption = ` Anti-delete-message by Beltah Tech254 🇰🇪\nMessage from @${senderId}`;
